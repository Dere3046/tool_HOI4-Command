...
<p align="center">
  <a href="README_zh.md">
    <img src="https://img.shields.io/badge/中文版本-Chinese%20Version-blue?style=for-the-badge" alt="Chinese Version">
  </a>
</p>
...
In the latest update, all commands now use their **full official names** as the primary keys in the database. While Hearts of Iron IV's console naturally supports command abbreviations (e.g., both `instantconstruction` and `ic` work identically in-game), this tool exclusively uses the complete official names for consistency and clarity.

- ✅ **Full Official Names Only**: All commands are stored and referenced by their complete names
- ✅ **Game Compatibility**: Commands generated by this tool work identically in-game regardless of abbreviation support
- ✅ **Clear Documentation**: Using full names makes the command database more readable and maintainable
...
**Important**: Always use the **full official command name** as the key, not abbreviations.
...
**Step 1 - Define command in `commands.json` (using full name):**
```json
"resource_management": {
    "add_fuel": {
...
All commands have been added to the database. You can search and use all official console commands within the tool.

**Note**: Due to game updates, some commands may change. We strive to keep the database updated, but we recommend referring to the official wiki for the most current information.
...
- **No Alias Support**: Only full official command names are supported (though game console naturally accepts abbreviations)
...
1. **Improve UI**: Enhance the user interface and experience  
2. **Add Features**: Implement requested features from users
3. **Fix Bugs**: Help identify and fix any issues
4. **Update Commands**: Maintain command database as the game updates
